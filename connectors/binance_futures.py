...
class BinanceFuturesClient:
    """
    Initializes the BinanceFuturesClient. Make testnet parameter True to avoid using real live data,
    ...
    def __init__(self, public_key: str, secret_key: str, testnet: bool = False):
        ...
        logger.info('BinanceFuturesClient successfully initialized')
    ...
    def _make_request(self, method: str, endpoint: str, data: typing.Dict):
        if method == 'GET':
            ...
            try:
                response = requests.delete(self._base_url + endpoint, data=data, headers=self._headers)
            except Exception as e:
                logger.error('Connection error while making %s request to %s: %s', method, endpoint, e)
                return None
        else:
    ...
    def _on_open(self, ws):
        ...
        self.subscribe_channel(list(self.contracts.values()),
                               'bookTicker')  # Max 200 channels, use '!bookTicker' for all in in single stream
